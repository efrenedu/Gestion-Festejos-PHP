...
?>
  ...
  <form onreset="reset_telefonos()"  id="formu" action="send_registro_trabajador.php" method="post">
     ...
     <select onchange="select_trabajador()" id='trabajadores' name='trabajadores' size='1' >
        ...
        <?php 
          ...
          if(validar_conexion()){
            ...
            $dat_admin=get_data_dict("usuario",["CI_trabaj"],1,["nombre_usuario"],["admin"]);
			$id_admin="000000";
		       ...
		       for ($i=0;$i<count($listado);$i++){
	               ...
	               if($cedula!="000000"){
				     if(count($data_name)>0){
				        ...
				        $nombre=$nombre." ".$data_name[0]["apellido"];
				        if($data_name[0]["segundo_apellido"]!=""){
					       $nombre=$nombre." ".$data_name[0]["segundo_apellido"];
				        }
     ...
     <select id='carg' name='cargo' size='1'>
	   ...
	   <option value='Obrero'>Artista</option>
